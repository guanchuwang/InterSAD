...
from robot_bases import Pose_Helper, Joint


class XmlBasedRobot_noisy:
  """
  ...
  def __init__(self, robot_name, action_dim, obs_dim, self_collision, noise_std=0):
    self.parts = None
    self.objects = []
    self.jdict = None
  ...
  def addToScene(self, bullet_client, bodies):
    ...
    for i in range(len(bodies)):
      if self._p.getNumJoints(bodies[i]) == 0:
        ...
        parts[part_name] = BodyPart_noisy(self._p, part_name, bodies, i, -1, noise_std=self.noise_std)
      ...
      for j in range(self._p.getNumJoints(bodies[i])):
        ...
        parts[part_name] = BodyPart_noisy(self._p, part_name, bodies, i, j, noise_std=self.noise_std)

        if part_name == self.robot_name:
          self.robot_body = parts[part_name]

        if i == 0 and j == 0 and self.robot_body is None:  # if nothing else works, we take this as robot_body
          parts[self.robot_name] = BodyPart_noisy(self._p, self.robot_name, bodies, 0, -1, noise_std=self.noise_std)
...
class MJCFBasedRobot_noisy(XmlBasedRobot_noisy):
  """
	Base class for mujoco .xml based agents.
	"""
  ...
  def __init__(self, model_xml, robot_name, action_dim, obs_dim, self_collision=True,
               noise_std=0):
    XmlBasedRobot_noisy.__init__(self, robot_name, action_dim, obs_dim, self_collision, noise_std=noise_std)
    self.model_xml = model_xml
    self.doneLoading = 0
  ...
  def reset(self, bullet_client):
    ...
    # print(dir(self.robot_body))
    # print(self.robot_body.state_fields_of_pose_of())
...
class BodyPart_noisy:

  def __init__(self, bullet_client, body_name, bodies, bodyIndex, bodyPartIndex, noise_std=0):
    self.bodies = bodies
    self._p = bullet_client
    ...
    # print(bullet_client, self.bodyIndex, bodyPartIndex, body_name)
  ...
  def state_fields_of_pose_of(
    ...
    return np.array([x, y, z, a, b, c, d]) # \
          # + np.random.randn(7,) * self.noise_std
