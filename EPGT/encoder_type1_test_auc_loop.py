...
roc_auc_buf = np.zeros((repeat_time, len(learn_step_buf)))
    ...
    for idx, learn_step in enumerate(learn_step_buf):
        ...
        for mdp_index in range(args.mdp_num):
            ...
            for step in range(args.trajectory_len):
                ...
                action_array = action_buf[step]
                action_tensor = torch.tensor(action_array, dtype=torch.float)
        ...
        feature = critic_encoding.reshape(args.mdp_num, -1).detach().numpy()
        ...
        roc_auc_buf[repeat, idx] = roc_auc
        print(learn_step, roc_auc)

    # print(roc_auc_buf[0:repeat+1, :].mean(axis=0))

roc_auc_mean = roc_auc_buf.mean(axis=0)
np.savetxt(load_dir + '/encoder-' + 'roc_auc.txt', roc_auc_mean)

time = np.array(learn_step_buf)
plt.plot(time, roc_auc_mean)
plt.xlabel('step')
plt.ylabel('ROC/AUC')
# plt.legend(loc='lower right')
plt.savefig(load_dir + '/encoder-' + args.exp + '-type' + str(args.type) + '.png')
plt.show()
