...
class Agent(object):
    ...
    def explore(self):
        ...
        self.state_action_time_series = torch.zeros((self.batch_size, self.trajectory_len, self.state_dim + self.action_dim))
        ...
        for batch_index in range(self.batch_size):
            ...
            for step in range(self.trajectory_len):
                ...
                state_noise = np.random.normal(0, self.state_noise_std, (self.state_dim,))
                nxt_state += state_noise  # self.state_normalizer(nxt_state)
                self.state_action_time_series[batch_index, step] = torch.cat((state_tensor, action_tensor), dim=0)
                state = nxt_state

        store_index = np.random.randint(0, self.batch_size)
        self.replay_mem.push(self.state_action_time_series[store_index].detach().unsqueeze(dim=0))

        # self.replay_mem.push(self.state_action_time_series.detach())
    ...
    def critic_learn(self):
        ...
        decoding = self.critic(state_action_time_series_batch_tensor)
        self.critic_loss = self.critic_loss_func(decoding, state_action_time_series_batch_tensor)
    ...
    def actor_learn(self):
        ...
        critic_encoding, critic_hidden, critic_cell = self.critic.hidden(self.state_action_time_series)
...
def main():
    ...
    mdps = MDPset(mdp_num=args.mdp_num,
                  ...
                  inlier_mdps=args.inlier_mdps,
                  outlier_mdps=args.outlier_mdps,
    ...
    for step in range(args.learn_step):
        ...
        if step >= args.warm_up:
            ...
            learn_step = step - args.warm_up
            if learn_step % args.save_interval == 0:
                agent.save_actor(args.save_dir + '/actor', learn_step)
                agent.save_critic(args.save_dir + '/critic', learn_step)

            agent.learn()

            if learn_step % args.report_interval == 0:
