...
class VirtualTB(gym.Env):
    ...
    def step(self, action):
        ...
        # print(self.total_a, self.total_c, self.cur_user, self.lst_action, self.rend_action, self.state)
        reward = int(self.lst_action[0])
        self.total_a += reward
        self.total_c += 1
        self.rend_action = deepcopy(self.lst_action)
        done = False # (self.total_c >= self.__leave)
        if done:
    ...
    def getuser(self):
        return (self.total_a, self.total_c, self.cur_user, self.lst_action, self.rend_action, self.state)

    def setuser(self, user_info):
        self.total_a = user_info[0]
        self.total_c = user_info[1]
        self.cur_user = user_info[2]
        self.lst_action = user_info[3]
        self.rend_action = user_info[4]
        return self.state
